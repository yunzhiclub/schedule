...
export class YzUploaderComponent implements OnInit {
  ...
  @Input()
  uploadFun = null as unknown as (file: File) => Observable<HttpEvent<any>>;
  ...
  emitFiles(event: FileList): void {
    ...
    if (this._multiple) {
      for (let i = 0; i < event.length; i++) {
        this.fileList.push(event.item(i) as File);
      }
    } else {
    ...
    this.fileList = this.fileList.filter(file => {
      if (file.size > config.uploadMaxSize) {
        this.commonService.error(() => {
          ...
          file.name + '的大小为'
          + Utils.fileSize(file.size)
          + '，超出了系统限制(' + Utils.fileSize(config.uploadMaxSize)
          + ')',
          '文件过大');
        return false;
      }
